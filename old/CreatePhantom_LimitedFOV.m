...
phantom_LimitedROI = ones(nx,ny,nz)*airHU;
mask0 = ((x-(1+nx)/2).^2/(60.^2)+(y-(1+ny)/2).^2/(50.^2)<1 & z<80 & z>20);
phantom_LimitedROI(mask0) = waterHU;
figure;imshow3D(phantom_LimitedROI,[-1000,1600])
...
for ii = 1:ninserts
    ...
    phantom_LimitedROI(mask) = mathu(ii);
end
...
PTV = 0*mask0;
PTV(phantom_LimitedROI==42516) = 1;
figure;imshow3D(PTV,[])

mkdir('D:\datatest\PairProd\phantom_LimitedROI\')
save('D:\datatest\PairProd\phantom_LimitedROI\phantom_LimitedROI.mat','phantom_LimitedROI','BODY','PTV')
