...
dose = reshape(full(M*x_),size(masks{1}.mask));
figure;imshow3D(dose,[])
...
%%
load('/media/raid1/qlyu/PairProd/datatest/CTphantom_360beam_10m_CTsim_blankfield/CTsim/dosematrix/Projection_beam.mat')
Projection_beam0 = Projection_beam;
...
LI = log(repmat(Projection_beam0,[1,1,size(Projection_beam,3)])./Projection_beam);

LI(isinf(LI)) = 0;
LI(isnan(LI)) = 0;
figure;imshow3D(LI,[]);
save(fullfile(dosematrixFolder,'Projection_beam.mat'),'Projection_beam','LI');
