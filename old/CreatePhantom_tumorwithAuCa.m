...
phantom_tumorwithAuCa = ones(nx,ny,nz)*airHU;
mask0 = ((x-(1+nx)/2).^2/(60.^2)+(y-(1+ny)/2).^2/(50.^2)<1 & z<80 & z>20);
phantom_tumorwithAuCa(mask0) = waterHU;
figure;imshow3D(phantom_tumorwithAuCa,[-1000,1600])
...
for ii = 1:ninserts
    ...
    phantom_tumorwithAuCa(mask) = mathu(ii);
end
...
mkdir('D:\datatest\PairProd\phantom_tumorwithAuCa\')
save('D:\datatest\PairProd\phantom_tumorwithAuCa\phantom_tumorwithAuCa.mat','phantom_tumorwithAuCa','BODY')
